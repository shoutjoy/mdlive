...
/* ═══════════════════════════════════════════════════════════
   MARKED SETUP — links new tab
═══════════════════════════════════════════════════════════ */
marked.setOptions({ breaks: true, gfm: true });
const _r = new marked.Renderer();
_r.heading = (text, level) => { const id = 'h-' + text.toLowerCase().replace(/[^a-z0-9가-힣\s]/g, '').replace(/\s+/g, '-').substring(0, 50); return `<h${level} id="${id}">${text}</h${level}>`; };
...
const IPPT = (() => {
    ...
    function setTool(t) {
        tool = t;
        /* pv-hdr + ppt-nav 모든 ippt-tool 버튼 업데이트 */
...
/* ═══════════════════════════════════════════════════════════
   ...
   297mm(A4 높이) 간격으로 빨간 점선을 각 .preview-page 위에 그림
   내용이 길어서 297mm를 넘는 페이지에도 반복 표시
═══════════════════════════════════════════════════════════ */
const A4Ruler = (() => {
    let on = false;

    /* 210mm 기준으로 실제 px scale 계산 */
    function getScale() {
    ...
    function drawOverlay() {
        ...
        const pvHdrH = 30;                           /* pv-hdr 높이 */

        /* container 기준 좌표로 각 페이지 위치 파악 */
        const containerRect = container.getBoundingClientRect();

        pages.forEach(page => {
            const pageRect = page.getBoundingClientRect();
            /* container 내 절대 y 좌표 (스크롤 포함) */
            const pageTop = pageRect.top - containerRect.top + containerScrollTop;
            const pageH = page.offsetHeight;

            /* 이 페이지 안에서 297mm 배수마다 선 */
            let n = 1;
            while (n * gap < pageH - 2) {
                const lineY = pageTop + n * gap;          /* overlay 기준 y */

                const line = document.createElement('div');
                line.className = 'a4-rl';
                /* overlay top = pv-hdr(30px), container top = pvHdrH 에서 시작
                   container 안 좌표 → overlay 좌표로 변환 */
  ...
  bi.addEventListener('scroll',()=>{
    ...
    function sync() {
        clearTimeout(st); st = setTimeout(() => {
            ...
            const title = el('doc-title').value;
            try {
                const sy = win.scrollY, sh = win.document.body.scrollHeight, ih = win.innerHeight, r = sy / Math.max(1, sh - ih);
                win.document.open(); win.document.write(buildHTML(el('editor').value, title)); win.document.close();
...
const SS = (() => {
    ...
    /* 에디터 라인 기반 헤딩 맵 빌드
       반환: [{line, id, edY}] — 에디터 스크롤 px 좌표 포함 */
    function buildEdMap(ed) {
        const lines = ed.value.split('\n');
        const lineH = ed.scrollHeight / Math.max(1, lines.length); /* 줄당 평균 높이 */
        const map = [];
        lines.forEach((ln, i) => {
    ...
    function pvAbsY(pc, id) {
        ...
        if (!h) return null;
        /* getBoundingClientRect 기반 — 스크롤 포함 절대값 */
...
const TOC = (() => {
    ...
    function build(md) {
        ...
        md.split('\n').forEach(line => { const m = line.match(/^(#{1,3})\s+(.+)/); if (!m) return; const lv = m[1].length, text = m[2].replace(/[*_`]/g, ''); const id = 'h-' + text.toLowerCase().replace(/[^a-z0-9가-힣\s]/g, '').replace(/\s+/g, '-').substring(0, 50); cnt[lv - 1]++; for (let i = lv; i < 3; i++)cnt[i] = 0; const num = lv === 1 ? cnt[0] : lv === 2 ? `${cnt[0]}.${cnt[1]}` : `${cnt[0]}.${cnt[1]}.${cnt[2]}`; hs.push({ lv, text, id, num }) });
        if (!hs.length) { list.innerHTML = '<div style="padding:12px;color:var(--tx3);font-size:12px">헤딩(#)을 추가하면 자동 생성됩니다.</div>'; return }
    ...
    /* 목차 자동 삽입 — 문서 맨 앞(또는 첫 h1 앞)에 마크다운 목차 블록 삽입 */
    function insertTOC() {
        const ed = el('editor');
        const md = ed.value;
...
const AS = (() => {
    ...
    function save(c, t2) {
        ...
        el('save-st').textContent = '저장 중...';
        t = setTimeout(() => {
            try {
                localStorage.setItem(K, JSON.stringify({ c, t: t2, ts: Date.now() }));
                el('save-dot').classList.remove('saving');
                el('save-dot').style.background = '';
                el('save-st').textContent = '저장됨';
            } catch (e) {
                // 이슈2 수정: QuotaExceededError 등 저장 실패 시 사용자에게 명시적으로 알림
                el('save-dot').classList.remove('saving');
                el('save-dot').style.background = '#f76a6a';
                el('save-st').textContent = '⚠ 자동저장 실패 (용량 초과) — 💾 저장 버튼으로 파일 저장 권장';
...
const ED = {
    ...
    image() { const alt = el('img-alt').value || '이미지'; const url = el('img-url').value || '#'; const ed = this.ed(), s = ed.selectionStart; ins(ed, s, s, `![${alt}](${url})`); el('img-alt').value = ''; el('img-url').value = ''; App.hideModal('image-modal') },
...
const HK = (() => {
    ...
    const DEFAULT_DATA = [
        {
            section: '문서 구조', items: [
                ...
                { desc: '페이지 나누기', keys: 'Ctrl + Enter' },
                { desc: '줄바꿈 (<br>)', keys: 'Ctrl + Shift + Enter' },
            ]
        },
        {
            section: '표 편집', items: [
                { desc: '표 삽입', keys: 'Alt + 8' },
                { desc: '행 추가', keys: 'Alt + 9' },
                { desc: '열 추가', keys: 'Alt + 0' },
                { desc: '가로 병합 (colspan)', keys: 'Alt + Shift + H' },
                { desc: '세로 병합 (rowspan)', keys: 'Alt + Shift + V' },
                { desc: 'HTML 표 들여쓰기 정돈', keys: 'Tidy 버튼' },
            ]
        },
        {
            section: '텍스트 서식', items: [
                ...
                { desc: '인라인 코드 `code`', keys: 'Alt + V' },
                { desc: '코드 직접 삽입 (마지막 언어)', keys: 'Alt + C' },
                { desc: '글자 크기 키우기', keys: 'Shift + Alt + .' },
                { desc: '글자 크기 줄이기', keys: 'Shift + Alt + ,' },
            ]
        },
        {
            section: '레이아웃 / 정렬', items: [
                { desc: '왼쪽 / 가운데 / 오른쪽 정렬', keys: 'Shift + Alt + L / C / R' },
                { desc: 'Split 보기', keys: 'Alt + 1' },
                { desc: '에디터만', keys: 'Alt + 2' },
                { desc: '미리보기만', keys: 'Alt + 3' },
            ]
        },
        {
            section: '편집', items: [
        ...
        {
            section: '삽입 / 도구', items: [
                { desc: '인용 삽입', keys: 'Ctrl + Shift + C' },
                { desc: '각주 삽입', keys: 'Shift + Alt + N' },
                { desc: 'APA 통계 삽입', keys: 'Shift + Alt + 9' },
                { desc: '서식 패널 (크기·색·형광펜)', keys: 'Alt + L' },
                { desc: '줄번호 ON/OFF', keys: 'Ctrl + Alt + I' },
                { desc: '새창 미리보기', keys: 'Ctrl + Shift + P' },
                { desc: '저장 다이얼로그', keys: 'Ctrl + S' },
                { desc: '찾기 / 바꾸기', keys: 'Ctrl + F' },
                { desc: 'Research Mode', keys: 'Ctrl + Shift + R' },
                { desc: 'Scholar 검색', keys: 'Ctrl + Shift + G' },
                { desc: '단축키 목록', keys: 'Alt + ?' },
            ]
        },
    ];
    ...
    function render() {
        ...
        data.forEach((group, gi) => {
            ...
            if (editMode) {
                ...
                addBtn.onclick = () => { data[gi].items.push({ desc: '새 항목', keys: '' }); render(); };
        ...
        if (editMode) {
            ...
            addSec.onclick = () => { data.push({ section: '새 섹션', items: [{ desc: '항목', keys: '' }] }); render(); };
    ...
    return {
        ...
        addRow() {
            ...
            data[data.length - 1].items.push({ desc: '새 항목', keys: '' });
            render();
        },
        saveEdit() {
            save();
...
function handleKey(e) {
    ...
    // Ctrl+Shift+Space → &nbsp; 삽입 (줄바꿈 다음 위치에 공백 추가)
    if (k === 'C+S+ ') {
        e.preventDefault();
...
const App = {
    ...
    render() {
        ...
        this.rt = setTimeout(() => {
            ...
            el('sw').textContent = words.toLocaleString() + ' 단어';
    ...
    updCursor() { const edi = el('editor'); const t = edi.value.substring(0, edi.selectionStart), ls = t.split('\n'); el('cursor-pos').textContent = `줄 ${ls.length}, 열 ${ls[ls.length - 1].length + 1}`; const sl = edi.selectionEnd - edi.selectionStart; el('sel-info').textContent = sl > 0 ? `${sl}자 선택` : '' },
    ...
    findNext() { const q = el('fi').value; if (!q) return; const edi = el('editor'); const idx = edi.value.indexOf(q, edi.selectionEnd); const fi2 = idx === -1 ? edi.value.indexOf(q) : idx; if (fi2 !== -1) { edi.setSelectionRange(fi2, fi2 + q.length); edi.focus() } const cnt = (edi.value.match(new RegExp(q.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')) || []).length; el('fc-cnt').textContent = cnt ? `${cnt}건` : q ? '없음' : '' },
    ...
    replaceAll() { const q = el('fi').value, r = el('ri').value; if (!q) return; const edi = el('editor'); const cnt = (edi.value.match(new RegExp(q.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')) || []).length; edi.value = edi.value.replaceAll(q, r); this.render(); US.snap(); el('fc-cnt').textContent = `${cnt}건 교체됨` },

    saveMD() { const c = el('editor').value, t = el('doc-title').value.replace(/[^a-z0-9가-힣]/gi, '_'); dlBlob(c, `${t}.md`, 'text/markdown'); this.hideModal('save-modal') },
    saveTXT() { const c = el('editor').value.replace(/[#*_`~>|]/g, '').replace(/\[(.*?)\]\(.*?\)/g, '$1').replace(/<[^>]+>/g, ''); dlBlob(c, (el('doc-title').value || 'document').replace(/[^a-z0-9가-힣]/gi, '_') + '.txt', 'text/plain;charset=utf-8'); this.hideModal('save-modal') },
    saveHTML() {
