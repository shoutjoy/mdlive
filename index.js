...
_r.heading = (text, level) => {
    /* text = marked가 파싱한 HTML (인라인 태그 + HTML 엔티티 포함)
       1) HTML 태그 제거
       2) HTML 엔티티 디코딩 (&amp; → & 등)
       3) TOC._calcId 와 동일한 공식으로 id 계산                  */
    const plain = text
        .replace(/<[^>]+>/g, '')
...
const TOC = (() => {
    let obs = null;
    /* ── id 생성: marked.js 와 동일한 방식으로 계산 ─────────
       marked 기본 renderer.heading 은 다음과 같이 id를 생성:
         text.toLowerCase().replace(/[^\w가-힣 ]/g,'').replace(/ /g,'-')
       단, marked 버전에 따라 다를 수 있으므로
       1차: 동일 공식으로 id 계산
       2차: DOM에서 텍스트 내용으로 실제 헤딩 요소를 직접 검색 (폴백)  */
    function _calcId(rawText) {
        /* renderer.heading 과 완전히 동일한 공식으로 id 계산
           - 마크다운 인라인 기호 제거
           - 'h-' 접두사 포함
           - substring(0,50) 동일하게 적용                    */
    ...
    function build(md) {
        ...
        md.split('\n').forEach(line => {
            ...
            const id   = _calcId(m[2]);   /* marked 와 동일 공식 */
            cnt[lv - 1]++;
    ...
    function go(id, rawText) {
        ...
        if (ed) {
            ...
            if (targetLine >= 0) {
                ...
                /* 편집창 scrollTop: 헤딩이 화면 위에서 약간 아래 오도록 */
...
const HK = (() => {
    ...
    const ACTION_MAP = {
        ...
        'ed.strikethrough': () => { const ed=el('editor'); if(ed){ const s=ed.selectionStart,e2=ed.selectionEnd; const sel=ed.value.slice(s,e2)||'텍스트'; const r=`~~${sel}~~`; ed.setRangeText(r,s,e2,'end'); US.snap(); TM.markDirty(); } },
        'ed.underline':     () => { const ed=el('editor'); if(ed){ const s=ed.selectionStart,e2=ed.selectionEnd; const sel=ed.value.slice(s,e2)||'텍스트'; ed.setRangeText(`<u>${sel}</u>`,s,e2,'end'); US.snap(); TM.markDirty(); } },
        'ed.sup':           () => { const ed=el('editor'); if(ed){ const s=ed.selectionStart,e2=ed.selectionEnd; ed.setRangeText(`<sup>${ed.value.slice(s,e2)||'텍스트'}</sup>`,s,e2,'end'); US.snap(); TM.markDirty(); } },
        'ed.sub':           () => { const ed=el('editor'); if(ed){ const s=ed.selectionStart,e2=ed.selectionEnd; ed.setRangeText(`<sub>${ed.value.slice(s,e2)||'텍스트'}</sub>`,s,e2,'end'); US.snap(); TM.markDirty(); } },
        'ed.highlight':     () => { const ed=el('editor'); if(ed){ const s=ed.selectionStart,e2=ed.selectionEnd; ed.setRangeText(`==${ed.value.slice(s,e2)||'텍스트'}==`,s,e2,'end'); US.snap(); TM.markDirty(); } },
...
(function () {
    ...
    //    → sw.js 파일이 있을 때만 등록, 없으면 기존 SW 해제
...
const Translator = (() => {
    ...
    async function _myMemory(text, sl, tl) {
        ...
        if (d.responseStatus !== 200)
            throw new Error(d.responseDetails || 'MyMemory 오류');
        const t = d.responseData.translatedText;
        /* MyMemory가 그대로 반환하거나 에러문 반환 시 예외 */
        if (!t || t === text) throw new Error('번역 결과 없음');
        return t;
    }
    ...
    async function translate() {
        ...
        if (out) out.innerHTML = '<span style="color:var(--tx3);font-style:italic">번역 중…</span>';
        ...
        try {
            ...
            if (out) out.innerHTML =
                `<span style="color:var(--er)">⚠ 번역 실패: ${e.message}</span>` +
                `<br><span style="font-size:11px;color:var(--tx3)">네트워크 상태 또는 언어 조합을 확인하세요.</span>`;
            _setStatus('❌ 오류', 'err');
        } finally {
            _busy = false;
    ...
    function insertResult() {
        ...
        const txt = mode === 'replace' ? _lastResult
                  : mode === 'after'   ? orig + _lastResult
    ...
    function clearInput() {
        ...
        _lastResult = '';
        const out = $('tr-output');
        if (out) out.innerHTML = '<span style="color:var(--tx3);font-style:italic">번역 결과가 여기에 표시됩니다.</span>';
  ...
  AppLock.init();
    ...
    return { show, hide, translate, swapLang, insertResult, copyResult, openBrowser, clearInput, onInput };
})();
